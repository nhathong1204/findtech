...
class Account(models.Model):
    ...
    account_id = ShortUUIDField(unique=True, length=10, max_length=25, prefix="217", alphabet="1234567890")
    ...
    def account_status_verbose(self):
        return dict(Account.ACCOUNT_STATUS)[self.account_status]
    
def handle_create_save_account(sender, instance, created, **kwargs):
    if created:
...
class KYC(models.Model):
    ...
    def nationality_verbose(self):
        return dict(KYC.NATIONALITY)[self.nationality]
    
    def marital_status_verbose(self):
        return dict(KYC.MARITAL_STATUS)[self.marital_status]
    
    def gender_verbose(self):
        return dict(KYC.GENDER)[self.gender]
    
    def identity_type_verbose(self):
        return dict(KYC.IDENTITY_TYPE)[self.identity_type]